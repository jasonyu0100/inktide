    ...
    return (
      <div className="space-y-2.5 my-5 print:hidden">
        {[100, 92, 78, 100, 65].map((w, i) => (
          <div
            key={i}
            className="h-3.5 rounded bg-white/[0.03] animate-pulse"
            style={{ width: `${w}%` }}
          />
        ))}
      </div>
...
function DeliveryCurveChart({ data }: { data: SlidesData }) {
  ...
  useEffect(() => {
    ...
    for (const p of eng.filter((e) => e.isPeak))
      g.append("path")
        .attr("d", d3.symbol().type(d3.symbolTriangle).size(28)())
    ...
    for (const v of eng.filter((e) => e.isValley))
      g.append("path")
        ...
        .attr(
          "transform",
          `translate(${x(v.index)},${y(v.smoothed) + 6}) rotate(180)`,
        )
        .attr("fill", "#93C5FD")
        .attr("opacity", 0.6);
...
function ForceDecompositionChart({ data }: { data: SlidesData }) {
  ...
  useEffect(() => {
    ...
    const x = d3
      ...
      .domain([0, n - 1])
      .range([0, w]);
    const maxVal = Math.max(...raw.fate, ...raw.world, ...raw.system, 1);
    const y = d3
      .scaleLinear()
      .domain([0, maxVal * 1.1])
      .range([h, 0]);
    for (const f of [
      { data: raw.system, color: "#3B82F6" },
      { data: raw.world, color: "#22C55E" },
      { data: raw.fate, color: "#EF4444" },
    ]) {
      const area = d3
        .area<number>()
        .x((_, i) => x(i))
...
function SwingChart({ data }: { data: SlidesData }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < swings.length; i++) {
      ...
      ma.push(
        swings.slice(start, i + 1).reduce((s, v) => s + v, 0) / (i - start + 1),
      );
    }
    g.append("path")
      .datum(ma)
...
function RadarChart({ data }: { data: SlidesData }) {
  ...
  useEffect(() => {
    ...
    const axes = [
      { key: "fate" as const, label: "F", angle: -Math.PI / 2 },
    ...
    for (const a of axes) {
      g.append("line")
        .attr("x1", 0)
        .attr("y1", 0)
        .attr("x2", Math.cos(a.angle) * maxR)
        .attr("y2", Math.sin(a.angle) * maxR)
        .attr("stroke", "white")
        .attr("stroke-opacity", 0.08);
      g.append("text")
        .attr("x", Math.cos(a.angle) * (maxR + 14))
        .attr("y", Math.sin(a.angle) * (maxR + 14))
        .attr("text-anchor", "middle")
        .attr("dominant-baseline", "middle")
        .attr("fill", FORCE_COLORS[a.key])
        .attr("font-size", 9)
    ...
    for (let i = 0; i < axes.length; i++)
      g.append("circle")
        ...
        .attr("r", 2.5)
        .attr("fill", FORCE_COLORS[axes[i].key]);
  }, [data]);
...
function MomentSparkline({
  ...
  useEffect(() => {
    ...
    if (winEnd - winStart < MOMENT_SPARKLINE_WINDOW - 1) {
      if (winStart === 0)
        winEnd = Math.min(full.length - 1, MOMENT_SPARKLINE_WINDOW - 1);
    ...
    g.append("circle")
      .attr("cx", x(sceneIdx))
      .attr("cy", y(full[sceneIdx]?.smoothed ?? 0))
      .attr("r", 3)
      .attr("fill", color)
      .attr("stroke", "white")
      .attr("stroke-width", 1);
    ...
    for (const e of eng.filter((e) => e.isPeak && e.index !== sceneIdx))
      g.append("path")
        ...
        .attr("transform", `translate(${x(e.index)},${y(e.smoothed) - 3})`)
        .attr("fill", "#FCD34D")
        .attr("opacity", 0.15);
    for (const e of eng.filter((e) => e.isValley && e.index !== sceneIdx))
      g.append("path")
        ...
        .attr(
          "transform",
          `translate(${x(e.index)},${y(e.smoothed) + 3}) rotate(180)`,
        )
        .attr("fill", "#93C5FD")
        .attr("opacity", 0.15);
...
function StateMachineGraph({ data }: { data: SlidesData }) {
  const { matrix, visitCounts, maxCount } = useMemo(() => {
    ...
    for (const from of CORNERS)
      for (const to of CORNERS)
        if (from !== to && m[from][to] > mc) mc = m[from][to];
    return { matrix: m, visitCounts: visits, maxCount: Math.max(mc, 1) };
  }, [data.forceSnapshots]);

  const maxVisits = Math.max(...Object.values(visitCounts), 1);
  const currentMode =
    data.forceSnapshots.length > 0
      ? detectCubeCorner(data.forceSnapshots[data.forceSnapshots.length - 1])
          .key
      : null;
  ...
  return (
    <svg viewBox={`0 0 ${GW} ${GH}`} className="w-full" style={{ height: 400 }}>
      ...
      {/* Edges */}
      {CORNERS.map((from) =>
        CORNERS.filter((to) => to !== from && matrix[from][to] > 0).map(
          (to) => {
            ...
            return (
              <line
                ...
                strokeWidth={1 + 3 * (count / maxCount)}
                opacity={opacity}
                markerEnd="url(#rpt-arrow)"
              />
            );
          },
        ),
...
const PROP_BASE_ORDER: PropositionBaseCategory[] = [
  "Anchor",
  "Seed",
  "Close",
  "Texture",
...
function ReportArcTrajectory({
  data,
  sceneProfiles,
}: {
  data: SlidesData;
  sceneProfiles: Map<string, Record<PropositionBaseCategory, number>> | null;
}) {
  ...
  for (const [, sceneIds] of arcMap) {
    let total = 0;
    const counts: Record<PropositionBaseCategory, number> = {
      Anchor: 0,
      Seed: 0,
      Close: 0,
      Texture: 0,
    ...
    <div className="grid grid-cols-4 gap-3 mb-6">
      {PROP_BASE_ORDER.map((base) => {
        ...
        return (
          <div
            key={base}
            className="rounded-lg p-3 border border-white/6 bg-white/2"
          >
            <div
              className="text-[9px] font-medium mb-2 lowercase"
...
export function NarrativeReport({
    ...
    for (const scene of data.scenes) {
      ...
      for (let bi = 0; bi < plan.beats.length; bi++) {
        ...
        for (let pi = 0; pi < beat.propositions.length; pi++) {
          ...
          if (cls)
            lc[classificationLabel(cls.base, cls.reach)] =
              (lc[classificationLabel(cls.base, cls.reach)] ?? 0) + 1;
        }
      }
    }
    return lc;
  ...
  const stats = {
    fate: {
      avg: avg(raw.fate),
      peak: Math.max(...raw.fate),
      total: raw.fate.reduce((s, v) => s + v, 0),
      sd: stdDev(raw.fate),
    },
    world: {
      avg: avg(raw.world),
      peak: Math.max(...raw.world),
      total: raw.world.reduce((s, v) => s + v, 0),
      sd: stdDev(raw.world),
    },
    system: {
      avg: avg(raw.system),
      peak: Math.max(...raw.system),
      total: raw.system.reduce((s, v) => s + v, 0),
      sd: stdDev(raw.system),
    },
    swing: {
      avg: avg(data.swings),
      peak: Math.max(...data.swings),
      total: data.swings.reduce((s, v) => s + v, 0),
      sd: stdDev(data.swings),
    },
  };
  const avgSwing = avg(data.swings);
  const swingVar = stdDev(data.swings);
  const pacingType =
    swingVar < avgSwing * 0.5
      ? "Steady"
      : swingVar > avgSwing * 1.2
        ? "Erratic"
        : "Varied";
        ...
        <div className="max-w-[680px] mx-auto px-6 py-14 report-body">
          ...
          <header className="mb-20 text-center report-cover">
            ...
            <div className="flex items-center justify-center gap-2 mt-8">
              <span className="text-[10px] px-2.5 py-1 rounded border border-white/[0.06] text-cyan-400/40">
                {data.scale.name}
              </span>
              <span className="text-[10px] px-2.5 py-1 rounded border border-white/[0.06] text-white/35">
                {data.shape.name}
              </span>
              <span className="text-[10px] px-2.5 py-1 rounded border border-white/[0.06] text-white/35">
                {data.archetype.name}
              </span>
              <span className="text-[10px] px-2.5 py-1 rounded border border-white/[0.06] text-emerald-400/40">
                {data.density.name}
              </span>
            </div>
            <div className="flex items-center justify-center gap-4 mt-4 text-[10px] text-white/20 font-mono">
              <span>{n} scenes</span>
              <span>{data.arcCount} arcs</span>
                ...
                <div className="space-y-1.5">
                  {forces.map((f) => (
                    <div key={f} className="flex items-center gap-2.5">
                      <span className="text-[10px] w-16 text-white/30">
                        {FORCE_LABELS[f]}
                      </span>
          ...
          <Section title="Delivery Curve" number={++sec}>
            ...
            <StatRow
              items={[
                {
                  label: "Peaks",
                  value: String(data.peaks.length),
                  accent: "text-amber-400/70",
                },
                {
                  label: "Valleys",
                  value: String(data.troughs.length),
                  accent: "text-blue-300/70",
                },
                {
                  label: "Shape",
                  value: data.shape.name,
                  accent: "text-white/60",
                },
                {
                  label: "Scale",
                  value: data.scale.name,
                  accent: "text-cyan-400/70",
                },
                {
                  label: "Density",
                  value: `${data.density.name} (${data.density.density}/s)`,
          ...
          <Section title="Force Analysis" number={++sec}>
                ...
                <thead>
                  <tr className="border-b border-white/[0.06]">
                    ...
                    {["Avg", "\u03C3", "Peak", "Total", "Grade"].map((h) => (
                      <th
                        key={h}
                        className="text-right py-2.5 px-2.5 text-[9px] uppercase tracking-[0.15em] text-white/20 font-normal"
                      >
                        {h}
                      </th>
                    ))}
                      ...
                      <tr key={f} className="border-b border-white/[0.03]">
                        <td className="py-3 pr-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              ...
                              style={{ color: FORCE_COLORS[f] + "CC" }}
                            >
                              {FORCE_LABELS[f]}
                            </span>
                          </div>
                        </td>
                        <td className="text-right py-3 px-2.5 text-[12px] font-mono text-white/50">
                          {s.avg.toFixed(2)}
                        </td>
                        <td className="text-right py-3 px-2.5 text-[12px] font-mono text-white/25">
                          {s.sd.toFixed(2)}
                        </td>
                        <td className="text-right py-3 px-2.5 text-[12px] font-mono text-white/40">
                          {s.peak.toFixed(2)}
                        </td>
                        <td className="text-right py-3 px-2.5 text-[12px] font-mono text-white/40">
                          {s.total.toFixed(1)}
                        ...
                        <td className="text-right py-3 px-2.5">
                          <span
                            ...
                            style={{
                              color: gradeColor(data.overallGrades[f], 25),
                            }}
                          >
                            {data.overallGrades[f]}
                          </span>
            ...
            <Figure caption="Raw force decomposition over time.">
              ...
              <div className="flex items-center gap-4 mt-3 px-1">
                {(["Fate", "World", "System"] as const).map((l) => (
                  <span
                    key={l}
                    className="flex items-center gap-1.5 text-[9px] text-white/25"
                  >
                    <span
                      className="w-3 h-[1.5px] rounded"
                      style={{
                        backgroundColor: FORCE_COLORS[l.toLowerCase()],
                        opacity: 0.7,
                      }}
                    />
                    {l}
              ...
              <div className="grid grid-cols-4 gap-3 mb-6">
                {(["Anchor", "Seed", "Close", "Texture"] as const).map(
                  (base) => {
                    ...
                    const pct =
                      data.propositionCount > 0
                        ? (count / data.propositionCount) * 100
                        : 0;
                    return (
                      <div
                        key={base}
                        className="text-center py-3 rounded-lg border border-white/6 bg-white/2"
                      >
                        <div
                        ...
                        <div
                          className="text-[9px] font-medium mt-0.5 lowercase"
                          style={{ color: BASE_COLORS[base] }}
                        >
                          {base}
                        </div>
                  ...
                  {ALL_PROFILE_LABELS.map(({ label, color }) => {
                    ...
                    return (
                      <div key={label} className="flex items-center gap-2">
                        <span
                          className="text-[9px] w-20 text-right font-medium"
                          style={{ color }}
                        >
                          {label}
                        </span>
                        ...
                        <span className="text-[8px] font-mono text-white/20 w-8">
                          {count}
                        </span>
                      </div>
                    );
          ...
          <Section title="Swing Analysis" number={++sec}>
            ...
            <StatRow
              items={[
                {
                  label: "Avg Swing",
                  value: avgSwing.toFixed(2),
                  accent: "text-yellow-400/70",
                },
                {
                  label: "Peak",
                  value: stats.swing.peak.toFixed(2),
                  accent: "text-white/50",
                },
                {
                  label: "\u03C3",
                  value: swingVar.toFixed(2),
            ...
            {data.segments.map((seg, segIdx) => {
              ...
              const segMoments = [
                ...data.peaks
                  .filter(
                    (p) =>
                      p.sceneIdx >= seg.startIdx && p.sceneIdx <= seg.endIdx,
                  )
                  .map((p) => ({
                    kind: "peak" as const,
                    sceneIdx: p.sceneIdx,
                    peak: p,
                    trough: null as (typeof data.troughs)[0] | null,
                  })),
                ...data.troughs
                  .filter(
                    (t) =>
                      t.sceneIdx >= seg.startIdx && t.sceneIdx <= seg.endIdx,
                  )
                  .map((t) => ({
                    kind: "valley" as const,
                    sceneIdx: t.sceneIdx,
                    peak: null as (typeof data.peaks)[0] | null,
                    trough: t,
                  })),
                ...
                <div key={segIdx} className={segIdx > 0 ? "mt-10" : "mt-2"}>
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    <span className="text-[10px] font-mono text-white/20">
                      Scenes {seg.startIdx + 1}&ndash;{seg.endIdx + 1}
                    </span>
                    <span
                      className="text-[9px] px-2 py-0.5 rounded capitalize"
                      style={{
                        color: FORCE_COLORS[seg.dominantForce] + "AA",
                        backgroundColor: FORCE_COLORS[seg.dominantForce] + "10",
                      }}
                    >
                  ...
                  {analysis?.segments?.[segIdx] && (
                    <p className="text-[13.5px] text-white/55 leading-[1.8] tracking-[0.01em] mb-4">
                      {analysis.segments[segIdx]}
                    </p>
                  )}
                  {analysisLoading && (
                      ...
                      {segMoments.map((m) => {
                        ...
                        return (
                          <div
                            ...
                            className="rounded-lg border border-white/[0.05] bg-white/[0.01] overflow-hidden report-moment"
                          >
                            <div
                              className={`flex items-center gap-3 px-4 py-2 ${isPeak ? "bg-amber-400/[0.03] border-b border-amber-400/[0.06]" : "bg-blue-400/[0.03] border-b border-blue-400/[0.06]"}`}
                            >
                              <span
                                className={`text-[9px] font-semibold uppercase tracking-[0.15em] ${isPeak ? "text-amber-400/60" : "text-blue-300/60"}`}
                              >
                                {isPeak ? "Peak" : "Valley"}
                              </span>
                              <span
                                className={`text-[11px] font-mono font-medium ${isPeak ? "text-amber-400/70" : "text-blue-300/70"}`}
                              >
                                Scene {m.sceneIdx + 1}
                              </span>
                ...
                <div className="space-y-2.5">
                  {data.topCharacters.slice(0, 8).map((c, i) => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-[10px] text-white/15 font-mono w-4 text-right">
                          {i + 1}
                        </span>
                          ...
                          <div className="h-[3px] rounded-full bg-white/[0.03] overflow-hidden">
                            <div
                              ...
                              style={{
                                ...
                                backgroundColor:
                                  c.character.role === "anchor"
                                    ? "#F59E0B"
                                    : c.character.role === "recurring"
                                      ? "#3B82F6"
                                      : "#6B7280",
                                opacity: 0.4,
                              }}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="h-[3px] rounded-full bg-white/[0.03] overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${(l.sceneCount / (data.topLocations[0]?.sceneCount ?? 1)) * 100}%`,
                                opacity: 0.3,
                              }}
                            />
                          </div>
                          {tiedNames.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-0.5">
                              {tiedNames.map((name) => (
                                <span
                                  key={name}
                                  className="text-[8px] text-white/25 bg-white/3 rounded px-1 py-px"
                                >
                                  {name}
                                </span>
                              ))}
                    ...
                    <div className="grid grid-cols-2 gap-x-8 gap-y-2.5">
                      {data.topArtifacts.slice(0, 8).map((a, i) => {
                        const owner = a.artifact.parentId
                          ? (narrative.characters[a.artifact.parentId]?.name ??
                            narrative.locations[a.artifact.parentId]?.name ??
                            null)
                          : null;
                        return (
                          <div
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="h-[3px] rounded-full bg-white/[0.03] overflow-hidden">
                                <div
                                  ...
                                  style={{
                                    width: `${maxUsage > 0 ? (a.usageCount / maxUsage) * 100 : 0}%`,
                                    opacity: 0.35,
                                  }}
                                />
                              </div>
                              <div className="flex flex-wrap gap-1 mt-0.5">
                                <span className="text-[8px] text-white/25 bg-white/3 rounded px-1 py-px">
                                  {owner ?? "world"}
                                </span>
          ...
          {data.threadLifecycles.length > 0 && (
            <Section title="Thread Portfolio" number={++sec}>
              ...
              <div className="mt-5 space-y-px rounded-lg overflow-hidden border border-white/[0.05]">
                {data.threadLifecycles.slice(0, 12).map((tl, i) => {
                  ...
                  return (
                    <div
                      key={tl.threadId}
                      className={`flex items-center gap-3 px-4 py-2 ${i % 2 === 0 ? "bg-white/[0.01]" : "bg-transparent"}`}
                    >
                      <span
                        className={`text-[11px] flex-1 min-w-0 truncate ${isTerminal ? "text-white/25" : "text-white/45"}`}
                      >
                        {tl.description}
                      </span>
                      <span className="text-[9px] font-mono text-white/15 shrink-0">
                        {firstScene + 1}&ndash;{lastScene + 1}
                      </span>
                      <span
              ...
              {data.threadLifecycles.length > 12 && (
                <p className="text-[10px] text-white/15 mt-2 pl-4">
                  +{data.threadLifecycles.length - 12} additional threads
                </p>
              )}
            </Section>
          )}
              ...
              {[...CORNERS]
                ...
                .map((c) => {
                  ...
                  return (
                    <div
                      key={c}
                      className="flex items-center gap-2 px-3 py-2.5 rounded border border-white/[0.04] bg-white/[0.01]"
                    >
                      <div
                      ...
                      <div className="min-w-0">
                        <div className="text-[10px] text-white/45 font-medium truncate">
                          {NARRATIVE_CUBE[c].name}
                        </div>
            ...
            {data.cubeTransitions.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-3">
                {data.cubeTransitions.slice(0, 8).map((t, i) => (
                  <span
                    key={i}
                    className="text-[9px] px-2 py-1 rounded bg-white/[0.02] border border-white/[0.04] text-white/25 font-mono"
                  >
                    <span style={{ color: CORNER_COLORS[t.from] + "99" }}>
                      {NARRATIVE_CUBE[t.from].name}
                    </span>
                    <span className="text-white/10 mx-0.5">{"\u2192"}</span>
                    <span style={{ color: CORNER_COLORS[t.to] + "99" }}>
                      {NARRATIVE_CUBE[t.to].name}
                    </span>
            ...
            <Section title="Arc Progression" number={++sec}>
              ...
              <div className="overflow-x-auto report-table">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/[0.06]">
                      <th className="text-left py-2 text-[9px] uppercase tracking-[0.15em] text-white/20 font-normal">
                        Arc
                      </th>
                      <th className="text-right py-2 text-[9px] uppercase tracking-[0.15em] text-white/20 font-normal w-12">
                        Sc.
                      </th>
                      {["P", "W", "S", "Sw"].map((h, i) => (
                        <th
                          key={h}
                          className="text-right py-2 text-[9px] font-normal w-10"
                          style={{
                            color:
                              [
                                FORCE_COLORS.fate,
                      ...
                      <th className="text-right py-2 text-[9px] uppercase tracking-[0.15em] text-white/20 font-normal w-14">
                        Score
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.arcGrades.map((arc, i) => (
                      ...
                      >
                        ...
                        <td className="text-right py-2.5 text-[11px] font-mono text-white/20">
                          {arc.sceneCount}
                        </td>
                        <td className="text-right py-2.5 text-[11px] font-mono text-white/30">
                          {arc.grades.fate}
                        </td>
                        <td className="text-right py-2.5 text-[11px] font-mono text-white/30">
                          {arc.grades.world}
                        ...
                        <td className="text-right py-2.5">
                          <span
                            ...
                            style={{
                              color: gradeColor(arc.grades.overall) + "CC",
                            }}
                          >
                            {arc.grades.overall}
                          </span>
