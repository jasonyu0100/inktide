...
function stationaryDist(matrix: BeatMatrix): Record<BeatFn, number> {
  ...
  for (const from of BEAT_FN_LIST) {
    ...
    rows[from] =
      total === 0
        ? (Object.fromEntries(BEAT_FN_LIST.map((c) => [c, uniform])) as Record<
            BeatFn,
            number
          >)
        : normalizeRow(matrix[from]);
  }
  let dist = Object.fromEntries(
    BEAT_FN_LIST.map((c) => [c, uniform]),
  ) as Record<BeatFn, number>;
...
export function BeatProfileSlide({ data }: { data: SlidesData }) {
  ...
  const { matrix, sequence, visitCounts, stationary, metrics } = useMemo(() => {
    ...
    for (const c of seq)
      if (visits[c as BeatFn] !== undefined) visits[c as BeatFn]++;
    ...
    return {
      ...
      metrics: {
        entropy,
        maxEntropy: Math.log2(10),
        selfLoopRate,
        setupFrac,
        driveFrac: 1 - setupFrac,
        observations,
        ...
        <div className="flex-1 flex flex-col gap-4 min-w-0">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] text-text-dim mt-1">
                How often consecutive beats repeat the same function. High =
                monotonous, low = every beat shifts.
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-[9px] text-text-dim mt-1">
                <span className="text-sky-400">Setup</span> = breathe, inform,
                expand, foreshadow, bond.{" "}
                <span className="text-red-400">Drive</span> = advance, turn,
                reveal, shift, resolve.
              </p>
            </div>
          </div>
