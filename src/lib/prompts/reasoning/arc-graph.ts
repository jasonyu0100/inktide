...
<node-types hint="Every node grounded in SPECIFIC context from the inputs above.">
  ...
  <type name="reasoning">A step in the causal chain — a distinct state-change (demand → plan, plan → action, action → consequence, consequence → new pressure). Same subject = one node with more edges. Minor restatements are pulses on the existing step — escalate or merge. Label = the inference (3-8 words).</type>
...
<requirements>
  ...
  <requirement index="15" name="terminal-commits">Last-indexed node advances state — closes a thread, lands a transformation, reveals a system truth, or hard-pivots to the next arc. Never a resting state.</requirement>
  <requirement index="16" name="novelty-over-recycling">Resolved threads mostly stay resolved — recycling is the exception. Prefer new threads of fate and new chains of reasoning over extending what already exists.</requirement>
</requirements>
...
<example>
{
  ...
  "nodes": [
    ...
    {"id": "reason-witness-exposure", "index": 5, "type": "reasoning", "label": "Negotiation exposes the protagonist to a hostile witness", "detail": "The path to sanctuary required revealing knowledge; a genuine setback the reasoning generated."},
    // order: 2 · index: 6 — new thread emerges from the cost.
    {"id": "fate-witness-leverage", "index": 6, "type": "fate", "label": "Witness now holds leverage over the protagonist", "detail": "New market opens at uniform prior; later arcs re-price.", "threadId": "T-NEW"},
    // order: 3 · index: 3 — the causal step.
    {"id": "reason-sanctuary-needs-alliance", "index": 3, "type": "reasoning", "label": "Sanctuary requires alliance with rival faction", "detail": "Backward reasoning from T-01's LEANS"},
