        ...
        >
          {items.map((item, i) => (
            ...
            >
              ...
              {item.shortcut && (
                <span className="text-[10px] text-text-dim/50 ml-4 font-mono">
                  {item.shortcut}
                </span>
              )}
            </button>
...
export default function TopBar() {
  ...
  const [pickerSections, setPickerSections] = useState<Record<string, boolean>>(
    { projects: true, works: false, playground: false },
  );
  ...
  useEffect(() => {
    ...
    function handleClick(e: MouseEvent) {
      if (
        ...
        !scorecardRef.current.contains(e.target as Node)
      ) {
      ...
      if (
        usageOpen &&
        usageRef.current &&
        !usageRef.current.contains(e.target as Node)
      ) {
        setUsageOpen(false);
      }
      if (
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      try {
        ...
        if (blob) {
          ...
          blobs.push({
            name: `${String(i + 1).padStart(3, "0")}_${arcName.replace(/[^a-z0-9]/gi, "_")}.mp3`,
            blob,
          });
        }
      } catch {
  ...
  const exportAvailability = useMemo(() => {
    ...
    return {
      ...
      hasPlans: allScenes.some(
        (s) => !!resolvePlanForBranch(s, branchId, branches),
      ),
  ...
  const scorecard = useMemo(() => {
    ...
    const perArc = arcsInOrder
      .map((arc) => {
        ...
        const arcFates = forceIndices.map((i) => raw.fate[i]);
        ...
        return {
          ...
          grades: gradeForces(arcFates, arcWorlds, arcSystem, arcSwingVals),
        };
      })
      ...
      <div className="flex items-center gap-0.5 text-sm min-w-0">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            <IconChevronDown
              size={12}
              className={`text-text-dim transition-transform ${selectorOpen ? "rotate-180" : ""}`}
            />
          </button>
            ...
            >
              <div className="max-h-80 overflow-y-auto py-1.5">
                {state.narratives.length === 0 ? (
                  ...
                  (() => {
                    const works = state.narratives.filter((n) =>
                      ANALYSIS_NARRATIVE_IDS.has(n.id),
                    );
                    const playground = state.narratives.filter((n) =>
                      PLAYGROUND_NARRATIVE_IDS.has(n.id),
                    );
                    const projects = state.narratives.filter(
                      (n) =>
                        !ANALYSIS_NARRATIVE_IDS.has(n.id) &&
                        !PLAYGROUND_NARRATIVE_IDS.has(n.id),
                    );
                    ...
                    return sections.map(({ key, label, entries }) => (
                      <div key={key}>
                        <button
                          onClick={() =>
                            setPickerSections((s) => ({ ...s, [key]: !s[key] }))
                          }
                          className="flex items-center gap-1.5 w-full px-3 py-1.5 text-left group"
                        >
                          <IconChevronRight
                            size={7}
                            className={`shrink-0 text-text-dim transition-transform ${pickerSections[key] ? "rotate-90" : ""}`}
                          />
                          <span className="text-[10px] font-semibold text-text-dim uppercase tracking-widest group-hover:text-text-secondary transition-colors">
                            {label}
                          </span>
                        ...
                        {pickerSections[key] &&
                          entries.map((entry) => {
                            const isActive =
                              state.activeNarrativeId === entry.id;
                            const isDeleting = deletingId === entry.id;
                            return (
                              <div key={entry.id}>
                                <div
                                  className={`flex items-center mx-1.5 rounded-lg transition-colors ${isActive ? "bg-white/8" : "hover:bg-white/5"}`}
                                >
                                  <button
                                    onClick={() => {
                                      setSelectorOpen(false);
                                  ...
                                  >
                                    <div className="text-[13px] text-text-primary truncate leading-snug">
                                      {entry.title}
                                    </div>
                                    <div className="text-[11px] text-text-dim truncate mt-0.5 leading-snug">
                                      {entry.description}
                                    </div>
                                  </button>
                                  <button
                                ...
                                {isDeleting && (
                                  <div
                                    className="mx-1.5 px-3 py-2.5 mb-1 rounded-lg"
                                    style={{
                                  ...
                                  >
                                    ...
                                    <button
                                      ...
                                      className="w-full text-xs font-medium py-1.5 rounded-md transition-colors bg-fate/20 text-fate hover:bg-fate/30 disabled:opacity-30 disabled:pointer-events-none"
                                    >
                                      Delete permanently
                                    </button>
                                  </div>
        ...
        <MenuDropdown
          ...
          items={[
            {
              label: "Force Analytics",
              onClick: () =>
                window.dispatchEvent(new Event("open-force-analytics")),
              disabled: !hasNarrative,
            },
            {
        ...
        <MenuDropdown
          ...
          items={[
            ...
            },
            {
              ...
              onClick: () => setMarkovOpen(true),
              disabled: !hasNarrative,
            },
            {
              label: "Beat Profile",
              onClick: () => setBeatProfileOpen(true),
              disabled: !hasNarrative,
            },
            {
              label: "Patterns",
            ...
            >
              ...
              <span
                className={`font-semibold font-mono ${
                  scorecard.grades.overall >= 90
                    ? "text-green-400"
                    : scorecard.grades.overall >= 80
                      ? "text-lime-400"
                      : scorecard.grades.overall >= 70
                        ? "text-yellow-400"
                        : scorecard.grades.overall >= 60
                          ? "text-orange-400"
                          : "text-red-400"
                }`}
              >
          ...
          {scorecardOpen && !scorecard && (
            <div className="absolute top-full right-0 mt-1 z-50 bg-bg-base border border-white/10 rounded-lg shadow-2xl p-5 w-[420px]">
              <p className="text-[12px] text-text-dim text-center py-4">
                No scenes yet — generate some arcs to see scores.
              </p>
            </div>
          )}
          {scorecardOpen && scorecard && (
            <div className="absolute top-full right-0 mt-1 z-50 bg-bg-base border border-white/10 rounded-lg shadow-2xl p-5 w-[420px]">
              ...
              <div className="flex items-center justify-between mb-1">
                <h2 className="text-[13px] font-semibold text-text-primary truncate max-w-[280px]">
                  {scorecard.title}
                </h2>
              ...
              <div className="grid grid-cols-[auto_1fr_1fr_1fr_1fr_1fr] gap-px bg-white/5 rounded overflow-hidden">
                ...
                {["Avg", "σ", "Peak", "Total", "Grade"].map((col) => (
                  <div key={col} className="bg-bg-base p-2 text-center">
                    <span
                      className={`text-[9px] tracking-wider text-text-dim font-mono ${col === "σ" ? "" : "uppercase"}`}
                    >
                      {col}
                    </span>
                  </div>
                ...
                {[
                  { key: "fate" as const, label: "Fate", color: "#EF4444" },
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                ...
                <div className="px-2 py-2 border border-white/5 rounded flex flex-col gap-1">
                  ...
                  <div className="flex items-center gap-2">
                    <ArchetypeIcon
                      archetypeKey={scorecard.archetype.key}
                      size={18}
                    />
              ...
              {scorecard.perArc.length > 1 &&
                (() => {
                  ...
                  const scoreColor = (v: number) => {
                    ...
                    if (v >= 80) {
                      const p = (v - 80) / 10;
                      return `rgb(${Math.round(163 - (163 - 34) * p)},${Math.round(230 + (197 - 230) * p)},${Math.round(53 + (94 - 53) * p)})`;
                    }
                    if (v >= 70) {
                      const p = (v - 70) / 10;
                      return `rgb(${Math.round(250 - (250 - 163) * p)},${Math.round(204 + (230 - 204) * p)},${Math.round(21 + (53 - 21) * p)})`;
                    }
                    if (v >= 60) {
                      const p = (v - 60) / 10;
                      return `rgb(${Math.round(249 + (250 - 249) * p)},${Math.round(115 + (204 - 115) * p)},${Math.round(22 - 22 * (1 - p))})`;
                    }
                          ...
                          <defs>
                            ...
                            >
                              {arcPoints.map((p, i) => (
                                <stop
                                  key={i}
                                  offset={`${(i / (arcPoints.length - 1)) * 100}%`}
                                  stopColor={scoreColor(p.score)}
                                />
                              ))}
                            </linearGradient>
                        ...
                        >
                          ...
                          {engPoints
                            ...
                            .map((p, i) => (
                              <polygon
                                key={i}
                                points={`${p.x},${p.y - 6} ${p.x - 4},${p.y - 1} ${p.x + 4},${p.y - 1}`}
                                fill="#FCD34D"
                                opacity="0.9"
                              />
                            ))}
            ...
            <div className="relative" ref={exportRef}>
              ...
              {exportOpen && (
                ...
                >
                  ...
                  <button
                    onClick={() => copyAllText("prose")}
                    disabled={!exportAvailability.hasProse}
                    className={`w-full flex items-center gap-2.5 px-3 py-1.5 text-[12px] transition-colors ${exportAvailability.hasProse ? "text-text-secondary hover:text-text-primary hover:bg-white/5" : "text-text-dim/50 cursor-not-allowed"}`}
                  >
                    <svg
                  ...
                  <button
                    onClick={() => copyAllText("plan")}
                    disabled={!exportAvailability.hasPlans}
                    className={`w-full flex items-center gap-2.5 px-3 py-1.5 text-[12px] transition-colors ${exportAvailability.hasPlans ? "text-text-secondary hover:text-text-primary hover:bg-white/5" : "text-text-dim/50 cursor-not-allowed"}`}
                  >
                    <svg
                      className="w-3.5 h-3.5 text-text-dim shrink-0"
                      viewBox="0 0 24 24"
                  ...
                  <button
                    onClick={() => copyAllText("summary")}
                    disabled={!exportAvailability.hasSummaries}
                    className={`w-full flex items-center gap-2.5 px-3 py-1.5 text-[12px] transition-colors ${exportAvailability.hasSummaries ? "text-text-secondary hover:text-text-primary hover:bg-white/5" : "text-text-dim/50 cursor-not-allowed"}`}
                  >
                    <svg
                      className="w-3.5 h-3.5 text-text-dim shrink-0"
                      viewBox="0 0 24 24"
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 px-3 py-1.5 text-[12px] transition-colors text-text-secondary hover:text-text-primary hover:bg-white/5"
                  >
                    <svg
                      className="w-3.5 h-3.5 text-text-dim shrink-0"
                      viewBox="0 0 24 24"
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-3 py-1.5 text-[12px] transition-colors ${exportAvailability.hasAudio ? "text-text-secondary hover:text-text-primary hover:bg-white/5" : "text-text-dim/50 cursor-not-allowed"}`}
                  >
                    <svg
                      className="w-3.5 h-3.5 text-text-dim shrink-0"
                      viewBox="0 0 24 24"
