...
type SortKey = "index" | "fate" | "world" | "system" | "proximity";
...
export function CubeExplorer({
  ...
  const [filterCorner, setFilterCorner] = useState<CubeCornerKey | "all">(
    "all",
  );
    ...
    if (scope === "local") {
      const currentIdx = Math.min(
        ...
        resolvedKeys
          ...
          .filter((k) => resolveEntry(narrative, k)?.kind === "scene").length -
          1,
      );
      const windowed = computeWindowedForces(
        allScenes,
  ...
  const displayed = useMemo(() => {
    let list =
      filterCorner === "all"
        ? entries
        : entries.filter((e) => e.corner === filterCorner);
    ...
    list = [...list].sort((a, b) => {
      ...
      switch (sortKey) {
        case "index":
          ...
          break;
        case "fate":
          cmp = a.forces.fate - b.forces.fate;
          break;
        case "world":
          cmp = a.forces.world - b.forces.world;
          break;
        case "system":
          cmp = a.forces.system - b.forces.system;
          break;
        case "proximity":
          cmp = a.proximity - b.proximity;
          break;
      }
    ...
    <Modal onClose={onClose} size="2xl" maxHeight="80vh" panelClassName="w-180">
        ...
        {(Object.keys(NARRATIVE_CUBE) as CubeCornerKey[]).map((key) => {
          ...
          return (
            <button
              key={key}
              onClick={() =>
                setFilterCorner(filterCorner === key ? "all" : key)
              }
      ...
      <div className="grid grid-cols-[40px_80px_1fr_60px_60px_60px_50px] gap-px px-5 py-2 border-b border-white/5 shrink-0 bg-white/[0.02]">
        {[
          ...
          { key: "fate" as SortKey, label: "F", width: "" },
      ...
      <div className="flex-1 overflow-y-auto">
        {displayed.length === 0 ? (
          <div className="py-12 text-center text-[11px] text-text-dim">
          ...
          displayed.map((entry) => {
            ...
            return (
              <div key={entry.scene.id}>
                <button
                  onClick={() =>
                    setExpandedId(isExpanded ? null : entry.scene.id)
                  }
                ...
                >
                  ...
                  <span className="flex items-center gap-1.5">
                    ...
                    <span
                      ...
                      style={{ color: CORNER_COLORS[entry.corner] }}
                    >
                      {entry.cornerName}
                    </span>
                  </span>
                  <span className="text-[11px] text-text-secondary truncate pr-2">
                    {entry.scene.summary}
                  </span>
                  <span className="text-[10px] font-mono text-[#EF4444]">
                    {entry.forces.fate.toFixed(1)}
                  </span>
                  <span className="text-[10px] font-mono text-[#22C55E]">
                    {entry.forces.world.toFixed(1)}
                  </span>
                  <span className="text-[10px] font-mono text-[#3B82F6]">
                    {entry.forces.system.toFixed(1)}
                  </span>
                  <span className="text-[10px] font-mono text-text-dim">
                    {(entry.proximity * 100).toFixed(0)}%
                  </span>
                </button>
                      ...
                      <div className="flex items-center gap-4 mb-3 text-[10px] text-text-dim">
                        {entry.arcName && (
                          <span className="flex items-center gap-1">
                            <span className="text-text-dim/50">Arc:</span>{" "}
                            {entry.arcName}
                          </span>
                        )}
                        {entry.locationName && (
                          <span className="flex items-center gap-1">
                            <span className="text-text-dim/50">Loc:</span>{" "}
                            {entry.locationName}
                          </span>
                        )}
                        {entry.povName && (
                          <span className="flex items-center gap-1">
                            <span className="text-text-dim/50">POV:</span>{" "}
                            {entry.povName}
                          </span>
                        )}
                      </div>
                        ...
                        <div className="mb-3">
                          ...
                          <ul className="space-y-0.5">
                            {entry.scene.events.map((evt, i) => (
                              <li
                                key={i}
                                className="text-[10px] text-text-secondary flex items-start gap-1.5"
                              >
                                <span className="text-text-dim/40 mt-0.5 shrink-0">
                                  &bull;
                                </span>
                                {evt}
                              </li>
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-[10px] text-text-dim">
                          {entry.scene.participantIds
                            .map(
                              (pid) => narrative.characters[pid]?.name ?? pid,
                            )
                            .join(", ")}
                        </span>
                      </div>
