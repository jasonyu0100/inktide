...
import { isScene, NARRATIVE_CUBE, resolveEntry } from "@/types/narrative";
...
function buildTransitionMatrix(
  scenes: { id: string; forces: ForceSnapshot }[],
): TransitionMatrix {
...
function computeMatrixMetrics(
  ...
  const sortedDist = CORNERS.slice().sort(
    (a, b) => (stationary[b] ?? 0) - (stationary[a] ?? 0),
  );
  const topPct = (stationary[sortedDist[0]] ?? 0) * 100;
  if (topPct > 35)
    observations.push(
      `${NARRATIVE_CUBE[sortedDist[0]].name} dominant at ${topPct.toFixed(0)}%.`,
    );
  if (selfLoopRate > 0.25)
    observations.push(
      `${(selfLoopRate * 100).toFixed(0)}% self-loop rate — tends to stay in modes.`,
    );
  const absent = CORNERS.filter((c) => (stationary[c] ?? 0) < 0.02);
  if (absent.length > 0)
    observations.push(
      `Absent: ${absent.map((c) => NARRATIVE_CUBE[c].name).join(", ")}.`,
    );
  const visitCounts: Record<CubeCornerKey, number> = {} as Record<
    CubeCornerKey,
    number
  >;
  ...
  if (absorbing.length > 0)
    observations.push(
      `Absorbing: ${absorbing.map((c) => NARRATIVE_CUBE[c].name).join(", ")}.`,
    );
  if (oscillationPairs.length > 0 && oscillationPairs[0].strength >= 4) {
    const p = oscillationPairs[0];
    observations.push(
      `${NARRATIVE_CUBE[p.a].name} ↔ ${NARRATIVE_CUBE[p.b].name} oscillation (${p.strength}×).`,
    );
  }
...
function TransitionGraph({
  ...
  const [hoveredCorner, setHoveredCorner] = useState<CubeCornerKey | null>(
    null,
  );
...
export function MarkovChainModal({
  ...
  const {
    ...
    const scenesWithForces = scenes.map((s) => ({
      id: s.id,
      forces: snapshots[s.id] || { fate: 0, world: 0, system: 0 },
    }));
    ...
    const total = CORNERS.reduce(
      (s, from) => s + CORNERS.reduce((s2, to) => s2 + mat[from][to], 0),
      0,
    );
    return {
      matrix: mat,
  ...
  const currentMode =
    cornerSeq.length > 0 ? cornerSeq[cornerSeq.length - 1] : null;
  ...
  const focusDetail = useMemo(() => {
    ...
    const outCount = Object.values(matrix[focusedCorner]).reduce(
      (s, v) => s + v,
      0,
    );
    ...
    return {
      ...
      sortedOut: CORNERS.map((c) => ({
        c,
        prob: outgoing[c],
        count: matrix[focusedCorner][c],
      }))
        .filter((o) => o.count > 0)
        .sort((a, b) => b.prob - a.prob),
      sortedIn: CORNERS.map((c) => ({
        c,
        prob: incoming[c],
        count: inCounts[c],
      }))
        ...
        {currentMode && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/5 text-[11px]">
            <div
              ...
              style={{ backgroundColor: CORNER_COLORS[currentMode] }}
            />
            <span className="text-text-dim">Now:</span>
            <span
              style={{ color: CORNER_COLORS[currentMode] }}
          ...
          <div className="px-5 py-4 space-y-2.5 border-b border-white/5">
            ...
            <div>
              <div className="flex justify-between text-[11px] mb-0.5">
                <span className="text-text-dim">Fate / Buildup</span>
                <span className="text-text-primary tabular-nums">
                  {(metrics.driveFrac * 100).toFixed(0)}% /{" "}
                  {(metrics.buildupFrac * 100).toFixed(0)}%
                </span>
              </div>
              <div className="w-full h-1.5 rounded-full bg-white/5 overflow-hidden flex">
                ...
                <div
                  ...
                  style={{ width: `${metrics.buildupFrac * 100}%` }}
                />
              </div>
            </div>
                ...
                {metrics.oscillationPairs.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {metrics.oscillationPairs.map(({ a, b, strength }) => (
                      ...
                      >
                        <span style={{ color: CORNER_COLORS[a] }}>
                          {NARRATIVE_CUBE[a].name}
                        </span>
                        <span className="text-text-dim">↔</span>
                        <span style={{ color: CORNER_COLORS[b] }}>
                          {NARRATIVE_CUBE[b].name}
                        </span>
            ...
            <div className="px-5 py-4 border-b border-white/5">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div
                    ...
                    style={{ backgroundColor: CORNER_COLORS[focusedCorner] }}
                  />
                  <span
                    className="text-[13px] font-semibold"
                    style={{ color: CORNER_COLORS[focusedCorner] }}
                  >
                    {NARRATIVE_CUBE[focusedCorner].name}
                  </span>
                </div>
                <button
              ...
              <div className="flex gap-4 mb-3 text-[11px]">
                ...
                <span>
                  ...
                  <span className="text-text-primary">
                    {(stationary[focusedCorner] * 100).toFixed(1)}%
                  </span>
                </span>
                <span>
                ...
                <span>
                  ...
                  <span className="text-text-primary">
                    {focusDetail.outCount > 0
                      ? `${(focusDetail.outgoing[focusedCorner] * 100).toFixed(0)}%`
                      : "—"}
                  </span>
                </span>
              </div>
                    ...
                    .map(({ c: to, prob, count }) => (
                      ...
                      >
                        <div
                          ...
                          style={{ backgroundColor: CORNER_COLORS[to] }}
                        />
                        <span
                          className="w-16 truncate"
                          style={{ color: CORNER_COLORS[to] }}
                        >
                          {NARRATIVE_CUBE[to].name}
                        </span>
                  ...
                  {focusDetail.sortedIn
                    ...
                    .map(({ c: from, prob, count }) => (
                      ...
                      >
                        <div
                          className="w-2 h-2 rounded-sm"
                          style={{ backgroundColor: CORNER_COLORS[from] }}
                        />
                        <span
                          className="w-16 truncate"
                          style={{ color: CORNER_COLORS[from] }}
                        >
            ...
            <div className="px-5 py-4 border-b border-white/5 space-y-4">
              ...
              {currentMode &&
                (() => {
                  ...
                  const sorted = CORNERS.map((c) => ({ c, prob: row[c] }))
                    ...
                    <div>
                      ...
                      {sorted.map(({ c, prob }) => (
                        ...
                        >
                          <div
                            className="w-2 h-2 rounded-sm"
                            style={{ backgroundColor: CORNER_COLORS[c] }}
                          />
                          <span
                            className="w-16 truncate"
                            style={{ color: CORNER_COLORS[c] }}
                          >
                ...
                <div className="flex items-end gap-1.5 h-16">
                  {CORNERS.map((c) => {
                    ...
                    return (
                      ...
                      >
                        {pct > 1 && (
                          <span className="text-[8px] tabular-nums text-text-dim">
                            {pct.toFixed(0)}%
                          </span>
                        )}
                        <div
                        ...
                        />
                        <span className="text-[7px] text-text-dim leading-tight text-center">
                          {NARRATIVE_CUBE[c].name}
                        </span>
                      </div>
                    );
                  ...
                  {CORNERS.map((from) => {
                    ...
                    return (
                      <tr
                        ...
                        onClick={() =>
                          setFocusedCorner(isFocused ? null : from)
                        }
                      >
                        <td
                          ...
                          style={{ color: CORNER_COLORS[from] }}
                        >
                          {NARRATIVE_CUBE[from].name.slice(0, 3)}
                        </td>
                        {CORNERS.map((to) => {
                          const prob = row[to];
                          return (
