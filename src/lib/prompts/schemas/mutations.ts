/**
 * Centralized JSON Schema Fragments for Mutations
 *
...
export const SCHEMA_THREAD_MUTATIONS = `"threadMutations": [{"threadId": "T-XX", "from": "latent|seeded|active|escalating|critical|resolved|subverted|abandoned", "to": "latent|seeded|active|escalating|critical|resolved|subverted|abandoned", "addedNodes": [{"id": "TK-XX", "content": "15-25 words, PRESENT tense: what just changed for this thread", "type": "pulse|transition|setup|escalation|payoff|twist|callback|resistance|stall"}]}]`;
...
export const SCHEMA_OWNERSHIP_MUTATIONS = `"ownershipMutations": [{"artifactId": "A-XX", "fromId": "C-XX or L-XX or null", "toId": "C-YY or L-YY or null"}]`;
...
export const SCHEMA_ANALYSIS_THREAD_MUTATIONS = `"threadMutations": [{"threadDescription": "exact thread description", "from": "latent|seeded|active|escalating|critical|resolved|subverted|abandoned", "to": "latent|seeded|active|escalating|critical|resolved|subverted|abandoned", "addedNodes": [{"content": "15-25 words, PRESENT tense: what just changed for this thread", "type": "pulse|transition|setup|escalation|payoff|twist|callback|resistance|stall"}]}]`;
...
export const SCHEMA_ANALYSIS_OWNERSHIP_MUTATIONS = `"ownershipMutations": [{"artifactName": "Name", "fromName": "prev owner or null", "toName": "new owner or null"}]`;
