/**
 * Force Standards Prompt — shared text block (used in both system and user
...
SYSTEM — ABSTRACT. Rules, principles, mechanisms. Not the things, but how they behave. Generic setting details don't count; only claims about how the world works. NAME the implicit mechanic — a cross-check bypassed reveals its structure, a humming artifact reveals a behaviour class. DENSITY LEVER — under-tagging system is the dominant generation failure.
  S = ΔN + √ΔE. Target ~${R.system}/scene. FLOOR: ≥1 structural fact per scene.

FATE — POSSIBILITY. The live space of what could still happen — each scene keeps some outcomes alive, closes others off, reshapes which branches are in play. Threads are PREDICTION MARKETS over named outcomes. Each scene's evidence shifts per-outcome logits; softmax gives the current probability distribution (the accounting layer — what the market prices right now). Fate per thread per scene equals the information gain (how much the market's belief moved) weighted by narrative attention (volumeDelta).
