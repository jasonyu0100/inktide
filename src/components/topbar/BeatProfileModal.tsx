...
import { BEAT_FN_LIST, isScene, resolveEntry } from "@/types/narrative";
...
function computeBeatMetrics(
  matrix: BeatMatrix,
  stationary: Record<BeatFn, number>,
  sequence: BeatFn[],
): BeatMetrics {
  ...
  const sortedDist = BEAT_FN_LIST.slice().sort(
    (a, b) => (stationary[b] ?? 0) - (stationary[a] ?? 0),
  );
  const topPct = (stationary[sortedDist[0]] ?? 0) * 100;
  if (topPct > 25)
    observations.push(`${sortedDist[0]} dominant at ${topPct.toFixed(0)}%.`);
  if (selfLoopRate > 0.15)
    observations.push(
      `${(selfLoopRate * 100).toFixed(0)}% self-loop rate — tends to repeat beats.`,
    );
  ...
  const absorbing = BEAT_FN_LIST.filter((c) => {
    const row = normalizeRow(matrix[c]);
    return row[c] > 0.3 && visitCounts[c] > 3;
  });
  if (absorbing.length > 0)
    observations.push(`Absorbing: ${absorbing.join(", ")}.`);
  if (oscillationPairs.length > 0 && oscillationPairs[0].strength >= 4) {
    const p = oscillationPairs[0];
    observations.push(
      `${p.a} \u2194 ${p.b} oscillation (${p.strength}\u00D7).`,
    );
  }
...
function BeatTransitionGraph({
  ...
  width: number;
  height: number;
  focusedFn: BeatFn | null;
  onFocusFn: (c: BeatFn | null) => void;
}) {
  ...
  return (
    <svg width={width} height={height} className="select-none">
      ...
      {BEAT_FN_LIST.map((from) =>
        BEAT_FN_LIST.filter((to) => to !== from && matrix[from][to] > 0).map(
          (to) => {
            ...
            const opacity = activeFn
              ? isRelevant
                ? Math.max(baseOpacity, 0.5)
                : 0.03
              : baseOpacity;
            return (
              <g key={`${from}-${to}`}>
                <line
                  ...
                  strokeWidth={1.5 + 3 * (count / maxCount)}
                  opacity={opacity}
                  markerEnd="url(#bp-arrow-heat)"
      ...
      {BEAT_FN_LIST.map((c) => {
        ...
        const isFocused = focusedFn === c;
        return (
          <g
            key={c}
            className="cursor-pointer transition-opacity duration-150"
            opacity={isHighlighted ? 1 : 0.2}
            onMouseEnter={() => setHoveredFn(c)}
...
export function BeatProfileModal({
  narrative,
  resolvedKeys,
  branchId,
  onClose,
  ...
  const { matrix, sequence, stationary, totalTransitions, mechDist, density } =
    useMemo(() => {
      ...
      if (seq.length === 0) {
        ...
        return {
          ...
          totalTransitions: 0,
          mechDist: {} as Partial<Record<BeatMechanism, number>>,
          density: 0,
        };
      }
      ...
      const total = BEAT_FN_LIST.reduce(
        (s, from) => s + BEAT_FN_LIST.reduce((s2, to) => s2 + mat[from][to], 0),
        0,
      );
      ...
      return {
        ...
        mechDist: sampler?.fnMechanismDistribution
          ? flattenFnMechDist(sampler.fnMechanismDistribution)
          : {},
  ...
  const metrics = useMemo(
    () => computeBeatMetrics(matrix, stationary, sequence),
    [matrix, stationary, sequence],
  );
  ...
  const focusDetail = useMemo(() => {
    ...
    const outCount = Object.values(matrix[focusedFn]).reduce(
      (s, v) => s + v,
      0,
    );
    ...
    for (const from of BEAT_FN_LIST)
      incoming[from] = inTotal > 0 ? inCounts[from] / inTotal : 0;
    ...
    return {
      ...
      sortedOut: BEAT_FN_LIST.map((c) => ({
        c,
        prob: outgoing[c],
        count: matrix[focusedFn][c],
      }))
        .filter((o) => o.count > 0)
        .sort((a, b) => b.prob - a.prob),
      sortedIn: BEAT_FN_LIST.map((c) => ({
        c,
        prob: incoming[c],
        count: inCounts[c],
      }))
        ...
        {currentFn && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/5 text-[11px]">
            <div
              ...
              style={{ backgroundColor: FN_COLORS[currentFn] }}
            />
            <span className="text-text-dim">Last:</span>
            <span
              style={{ color: FN_COLORS[currentFn] }}
        ...
        <div className="flex-1 flex min-h-0">
          ...
          <div className="w-[420px] border-l border-white/5 shrink-0 overflow-y-auto">
            ...
            <div className="px-5 py-4 space-y-2.5 border-b border-white/5">
              <MetricBar
                label="Variety"
                value={`${metrics.entropy.toFixed(2)} / ${metrics.maxEntropy.toFixed(2)}`}
                pct={(metrics.entropy / metrics.maxEntropy) * 100}
                color="bg-violet-500/70"
              />
              <MetricBar
                label="Self-loops"
                value={`${(metrics.selfLoopRate * 100).toFixed(0)}%`}
                pct={metrics.selfLoopRate * 100}
              ...
              <div>
                <div className="flex justify-between text-[11px] mb-0.5">
                  ...
                  <span className="text-text-primary tabular-nums">
                    {(metrics.setupFrac * 100).toFixed(0)}% /{" "}
                    {(metrics.driveFrac * 100).toFixed(0)}%
                  </span>
                </div>
                <div className="w-full h-1.5 rounded-full bg-white/5 overflow-hidden flex">
                  <div
                    className="h-full bg-sky-500/70"
                    style={{ width: `${metrics.setupFrac * 100}%` }}
                  />
                  <div
                    className="h-full bg-red-500/70"
                    style={{ width: `${metrics.driveFrac * 100}%` }}
                  />
                </div>
              </div>
                    ...
                    {Object.entries(mechDist)
                      ...
                      .map(([mech, pct]) => (
                        <div key={mech} className="flex items-center gap-1.5">
                          <span
                            className="text-[9px] font-mono w-14 shrink-0"
                          ...
                          <span className="text-[9px] text-text-dim tabular-nums w-7 text-right">
                  ...
                  {metrics.oscillationPairs.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {metrics.oscillationPairs.map(({ a, b, strength }) => (
                        <span
                          key={`${a}-${b}`}
                          className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-white/5 text-[9px]"
                        >
            ...
            {focusDetail && focusedFn ? (
              <div className="px-5 py-4 border-b border-white/5">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <div
                      ...
                      style={{ backgroundColor: FN_COLORS[focusedFn] }}
                    />
                    <span
                      className="text-[13px] font-semibold"
                      style={{ color: FN_COLORS[focusedFn] }}
                    >
                      {focusedFn}
                    </span>
                  </div>
                  <button
                    onClick={() => setFocusedFn(null)}
                    className="text-[10px] text-text-dim hover:text-text-primary px-2 py-0.5 rounded hover:bg-white/5"
                  >
                    Clear
                  </button>
                </div>
                <div className="flex gap-4 mb-3 text-[11px]">
                  ...
                  <span>
                    <span className="text-text-dim">Eq </span>
                    <span className="text-text-primary">
                      {(stationary[focusedFn] * 100).toFixed(1)}%
                    </span>
                  </span>
                  <span>
                    <span className="text-text-dim">Dwell </span>
                    <span className="text-text-primary">
                      {focusDetail.avgDwell}
                    </span>
                  </span>
                  <span>
                    <span className="text-text-dim">Loop </span>
                    <span className="text-text-primary">
                      {focusDetail.outCount > 0
                        ? `${(focusDetail.outgoing[focusedFn] * 100).toFixed(0)}%`
                        : "\u2014"}
                    </span>
                  </span>
                </div>
                      ...
                      .map(({ c: to, prob }) => (
                        ...
                        >
                          <div
                            ...
                            style={{ backgroundColor: FN_COLORS[to] }}
                          />
                          <span
                            className="w-16 truncate"
                            style={{ color: FN_COLORS[to] }}
                          >
                            {to}
                          </span>
                          ...
                          <span className="text-text-dim tabular-nums w-8 text-right text-[10px]">
                            {(prob * 100).toFixed(0)}%
                          </span>
                        </div>
                      ))}
                    ...
                    {focusDetail.sortedIn
                      ...
                      .map(({ c: from, prob }) => (
                        ...
                        >
                          <div
                            className="w-2 h-2 rounded-sm"
                            style={{ backgroundColor: FN_COLORS[from] }}
                          />
                          <span
                            className="w-16 truncate"
                            style={{ color: FN_COLORS[from] }}
                          >
                            {from}
                          </span>
                          <div className="flex-1 h-1.5 rounded-full bg-white/5 overflow-hidden">
              ...
              <div className="px-5 py-4 border-b border-white/5 space-y-4">
                ...
                {currentFn &&
                  (() => {
                    ...
                    const sorted = BEAT_FN_LIST.map((c) => ({
                      c,
                      prob: row[c],
                    }))
                      ...
                      <div>
                        ...
                        {sorted.map(({ c, prob }) => (
                          ...
                          >
                            <div
                              className="w-2 h-2 rounded-sm"
                              style={{ backgroundColor: FN_COLORS[c] }}
                            />
                            <span
                              className="w-16 truncate"
                              style={{ color: FN_COLORS[c] }}
                            >
                              {c}
                            </span>
                            <div className="flex-1 h-1.5 rounded-full bg-white/5 overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${prob * 100}%`,
                                  backgroundColor: FN_COLORS[c],
                                  opacity: 0.7,
                                }}
                              />
                            </div>
                            <span className="text-text-dim tabular-nums text-[10px] w-8 text-right">
                              {(prob * 100).toFixed(0)}%
                            </span>
                          </div>
                        ))}
                ...
                <div>
                  ...
                  <div className="flex items-end gap-1 h-16">
                    {BEAT_FN_LIST.map((c) => {
                      ...
                      const maxPct =
                        Math.max(...Object.values(stationary)) * 100;
                      ...
                      return (
                        ...
                        >
                          {pct > 1 && (
                            <span className="text-[7px] tabular-nums text-text-dim">
                              {pct.toFixed(0)}%
                            </span>
                          )}
                          <div
                          ...
                          />
                          <span className="text-[6px] text-text-dim leading-tight text-center truncate w-full">
                            {c}
                          </span>
                        </div>
                      );
                ...
                <table className="w-full text-[9px] border-collapse">
                  <thead>
                    <tr>
                      ...
                      {BEAT_FN_LIST.map((c) => (
                        <th
                          key={c}
                          className="p-0.5 text-center font-medium"
                          style={{ color: FN_COLORS[c] }}
                        >
                          {c.slice(0, 3)}
                        </th>
                  ...
                  <tbody>
                    {BEAT_FN_LIST.map((from) => {
                      ...
                      const totalCount = Object.values(matrix[from]).reduce(
                        (s, v) => s + v,
                        0,
                      );
