...
export const GENERATE_NARRATIVE_SYSTEM =
  'You are a narrative architect spinning a complete seed from a title + premise. Build a tight, focused world with named entities (characters / scenario actors / modelled agents with secrets or hidden state, locations / theatres / model regions with constraints, threads with named outcomes, system rules with mechanisms — for simulation register the rule set is load-bearing, not flavour), all woven together. Full mode: also produce an 8-scene opening arc + prose profile. World-only mode: entities + system + prose profile, no scenes. Source names from the cultural palette the premise actually implies — when the premise points elsewhere don\'t reflexively reach for Anglo/Celtic/Greek, but when the premise IS Western (US politics, European history, an English-language memoir) Western names are the correct answer. Initialize every entity with seed nodes; never emit blank world graphs. Return ONLY valid JSON matching the schema in the user prompt.';
...
<rules name="opening-arc" hint="Establish a tight, focused world. Counts below are minimums; exceed when the premise warrants it.">
  ...
  <entity-definitions>
    <entity id="characters">Named beings with agency — people, named animals, sentient AI (AGI), authors, investigators, named sources, subjects, witnesses. In simulation register: scenario actors (commanders, ministers, traders, epidemiologists, modelled cultivators), decision-making agents whose choices feed the rule machinery, observers running the model. Non-sentient AI systems are artifacts.</entity>
    <entity id="locations">Spatial areas, regions, or institutions — places you can be IN, physically or institutionally (a city, an archive, a laboratory, a movement). In simulation register: scenario theatres (a Mughal subah, a strait, a quarantined district), model regions (a population grid, a market zone), agent populations treated as a place (a guild, a sect, a faction's catchment).</entity>
    <entity id="artifacts">Anything that delivers utility — tools, documents, instruments, sources. Active, not passive concepts. Concepts belong in system knowledge. In simulation register: rule documents (treaties, doctrinal texts, statutes), instruments (sensors, telegrams, audit ledgers), scenario inputs (calibration datasets, mobility tables), model outputs (forecast bulletins, dashboards, status sheets) when characters in-world consult them.</entity>
    <entity id="threads">
