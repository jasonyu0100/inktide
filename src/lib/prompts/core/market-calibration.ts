 ...
 *
 * These blocks are PLAIN TEXT — they are concatenated into BOTH system and
 * user prompts, so they cannot use XML wrappers (system prompts stay text).
 */
...
REGISTER-AGNOSTIC (translate as needed):
- Fiction: central agent = protagonist, step = scene, segment = arc.
- Argument or inquiry: central agent = committed hypothesis, step = section, segment = chapter.
- Reportage: central agent = reporter's claim, step = finding, segment = investigation.
...
export const PROMPT_MARKET_LOGTYPE_TABLE = `LOGTYPE TABLE — logType MUST agree with direction + magnitude.
- setup (evidence +0..+1): planting, low prior.
