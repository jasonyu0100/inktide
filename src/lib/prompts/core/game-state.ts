 ...
 *
 * Plain text — used in both system and user prompt contexts.
 */

export const PROMPT_ARC_STATE_GUIDANCE = `ARC METADATA — DIRECTION VECTOR (forward) & WORLD STATE (backward) frame every arc.

Direction looks forward — what this arc drives toward. World State looks backward — the compact, objective position after the arc resolves, from which a downstream reasoner can pick up WITHOUT replaying the scene deltas.

FIELDS:
- directionVector (10-15 words): single sentence; uses ENTITY NAMES; states what changes, who drives it, what's at stake.
- worldState (50-90 words): terse and structured, ground truth only, no speculation, no narration. The "chess-board position" as of the END OF THIS ARC.

DOMAIN-ADAPTIVE — identify the TYPE OF WORK first, then emit state in that domain's NATIVE compact form. Do NOT force narrative shape onto a chess position, or stats onto a story.
...
- Poker / imperfect info: stack sizes, pot size, hole cards if known, community cards, action position, inferred ranges.
- Paper / non-fiction / research: claims established, evidence anchored, open questions, unresolved dependencies, remaining work.
