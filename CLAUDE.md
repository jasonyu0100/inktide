...
- **Change (C)** — mutation reach per character with logarithmic scaling. Formula: `Σ_c log₂(1 + m_c)`. Counts all mutations (continuity, relationship, thread) per character. Captures breadth (characters affected) and depth (mutations per character).
