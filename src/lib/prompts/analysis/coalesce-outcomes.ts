...
11. MECE CHECK — canonical set must be (a) DISJOINT (no two can both be true; ✗ "instability persists" + "new major conflict" co-occur → re-partition to "no conflict / one / multiple") and (b) EXHAUSTIVE (covers every live future). You MAY re-partition the axis during coalescing and pick canonical labels that don't match any input literally — every input variant still maps to exactly one canonical via merges.
12. NEUTRAL LABELS — canonicals name observable future-states, not slogans. ✓ "US settles into multipolar order"; ✗ "US reasserts pre-eminence" when the source argued the unipolar moment is over. A variant whose name encodes a position the source rejected does NOT survive as a canonical — map it into the nearest neutral canonical.
13. REJECT TRIVIAL CANONICALS. Labels containing "complex", "significant", "meaningful", "important", "notable", "has effect", "matters", "turns out to be important" without concrete referent content are trivially-true — they describe that SOMETHING happens, not what. ✗ "reveals complex connection" / "has meaningful effect" / "turns out to be significant" → if the inputs give enough signal, re-partition into concrete alternatives ("they share an ancestor" / "they trained under the same master" / "they have no connection"). If the inputs are all meta-observations with no concrete referent, the original outcome set was under-specified; coalesce into the least-bad variants and prefer a specific "other" residual over a meta-canonical. A market priced on a trivially-true canonical can only close at low quality and loses the structural continuity the thread was meant to track.
