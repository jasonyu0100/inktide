...
import { applyLogMutation, EMPTY_THREAD_LOG } from '@/lib/thread-log';
import type { LogMutation } from '@/types/narrative';
...
describe('applyLogMutation', () => {
  ...
  it('silently drops duplicate IDs — the invariant TK-ID remaps depend on', () => {
    ...
    // globally unique TK-NNN before calling applyLogMutation.
    const first: LogMutation = {
      threadId: 'T-01', from: 'active', to: 'active',
      addedNodes: [{ id: 'TK-01', content: 'original', type: 'pulse' }],
    };
    const second: LogMutation = {
      threadId: 'T-01', from: 'active', to: 'active',
      addedNodes: [{ id: 'TK-01', content: 'duplicate — should be dropped', type: 'pulse' }],
    };
    let log = applyLogMutation(EMPTY_THREAD_LOG, first);
    log = applyLogMutation(log, second);
  ...
  it('idempotent on re-application of the same mutation (supports store replay)', () => {
    ...
    // applied repeatedly. applyLogMutation must be idempotent with
    ...
    const once = applyLogMutation(EMPTY_THREAD_LOG, mutation);
    const twice = applyLogMutation(once, mutation);
  ...
  it('handles missing addedNodes field defensively', () => {
    // LogMutation type requires addedNodes, but defensive code in
    // applyLogMutation uses `?? []` to handle legacy/malformed input.
    const mutation = {
      threadId: 'T-01', from: 'active', to: 'active',
    } as unknown as LogMutation;
    const log = applyLogMutation(EMPTY_THREAD_LOG, mutation);
    expect(Object.keys(log.nodes)).toHaveLength(0);
  });
  ...
  it('defaults missing node type to pulse', () => {
    const mutation = {
      threadId: 'T-01', from: 'active', to: 'active',
      addedNodes: [{ id: 'TK-01', content: 'no type' } as unknown as LogMutation['addedNodes'][number]],
    };
    const log = applyLogMutation(EMPTY_THREAD_LOG, mutation);
