  ...
  if (access.userApiKeys && !access.hasOpenRouterKey) {
    return (
      <div className="flex h-full items-center justify-center flex-col gap-2">
        <p className="text-xs text-text-dim">OpenRouter API key required for chat</p>
        <button
  ...
  if (!state.activeNarrative) {
    return (
      <div className="flex h-full items-center justify-center">
        <p className="text-sm text-text-dim">Open a narrative to chat</p>
      </div>
    );
  }
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-3 py-2 space-y-3 min-h-0">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-2 text-center">
            <p className="text-xs text-text-dim">Ask about the story at this point in the timeline</p>
            <div className="flex flex-wrap gap-1 justify-center max-w-[200px]">
              {['What threads are active?', 'Suggest a next scene', 'Who knows what?'].map((q) => (
                <button
                  key={q}
