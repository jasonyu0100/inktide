...
function normalizeRow(
  row: Record<CubeCornerKey, number>,
...
function stationaryDist(
  ...
  for (const from of CORNERS) {
    ...
    rows[from] =
      total === 0
        ? (Object.fromEntries(CORNERS.map((c) => [c, uniform])) as Record<
            CubeCornerKey,
            number
          >)
        : normalizeRow(matrix[from]);
  }
  let dist = Object.fromEntries(CORNERS.map((c) => [c, uniform])) as Record<
    CubeCornerKey,
    number
  >;
...
export function PacingProfileSlide({ data }: { data: SlidesData }) {
  ...
  const { matrix, visitCounts, stationary, metrics } = useMemo(() => {
    ...
    return {
      ...
      metrics: {
        entropy,
        maxEntropy: Math.log2(8),
        selfLoopRate,
        driveFrac,
        buildupFrac: 1 - driveFrac,
        observations,
        oscillationPairs: oscPairs.slice(0, 3),
      },
    };
  }, [data.forceSnapshots]);
    ...
    <div className="flex flex-col h-full px-10 py-6 overflow-y-auto">
      <div className="flex gap-6 items-center flex-1 min-h-0">
            ...
            {CORNERS.map((from) =>
              CORNERS.filter((to) => to !== from && matrix[from][to] > 0).map(
                (to) => {
                  ...
                  const fromR =
                    baseR + (visitCounts[from] / maxVisits) * maxExtraR;
                  const sx =
                    p1.x + dx * Math.min(1, (fromR + 8) / len) + 5 * nx;
                  const sy =
                    p1.y + dy * Math.min(1, (fromR + 8) / len) + 5 * ny;
                  const ex =
                    p1.x + dx * Math.max(0, (len - toR - 8) / len) + 5 * nx;
                  const ey =
                    p1.y + dy * Math.max(0, (len - toR - 8) / len) + 5 * ny;
              ...
              return (
                ...
                >
                  <circle
                    ...
                    opacity={0.9}
                    stroke={hovered === c ? "#fff" : "transparent"}
                    strokeWidth={2}
                  />
                  <text
                    x={pos.x}
        ...
        <div className="flex-1 flex flex-col gap-4 min-w-0">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-xs font-mono text-text-primary">
                  {metrics.entropy.toFixed(2)} / {metrics.maxEntropy.toFixed(2)}
                </span>
              </div>
              <div className="h-1.5 rounded-full bg-white/5 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${(metrics.entropy / metrics.maxEntropy) * 100}%`,
                  }}
                />
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-xs font-mono text-text-primary">
                  {(metrics.selfLoopRate * 100).toFixed(0)}%
                </span>
              </div>
              <div className="h-1.5 rounded-full bg-white/5 overflow-hidden">
                <div
                  className="h-full rounded-full bg-amber-400"
                  style={{ width: `${metrics.selfLoopRate * 100}%` }}
                />
              </div>
              <p className="text-[9px] text-text-dim mt-1">
                How often consecutive scenes stay in the same mode. High =
                sticky, low = every scene shifts.
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-[9px] text-text-dim mt-1">
                <span className="text-red-400">Fate</span> = Epoch, Climax,
                Revelation, Closure.{" "}
                <span className="text-sky-400">Buildup</span> = Discovery,
                Growth, Lore, Rest.
              </p>
            </div>
          </div>
          ...
          {metrics.oscillationPairs.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {metrics.oscillationPairs.map((p, i) => (
                <span
                  key={i}
                  className="inline-flex items-center gap-1 rounded-full bg-white/5 px-2.5 py-1 text-[10px] text-text-secondary"
                >
                  <span style={{ color: CORNER_COLORS[p.a] }}>
                    {NARRATIVE_CUBE[p.a].name}
                  </span>
                  <span className="text-text-dim">&#x21C4;</span>
                  <span style={{ color: CORNER_COLORS[p.b] }}>
                    {NARRATIVE_CUBE[p.b].name}
                  </span>
          ...
          {/* Equilibrium */}
          <div>
            <span className="text-[10px] text-text-dim uppercase tracking-wider block mb-2">
              Equilibrium
            </span>
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{
                          ...
                          opacity: pct > 1 ? 0.8 : 0.2,
                        }}
                      />
                      <span className="text-[7px] text-text-dim truncate w-full text-center">
                        {NARRATIVE_CUBE[c].name}
                      </span>
                    </div>
                  );
