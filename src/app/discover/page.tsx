...
function applySysUpdates(
  existing: PremiseSystemSketch[],
  newSystems: PremiseSystemSketch[],
  updates: {
    name: string;
    addPrinciples?: string[];
  ...
  for (const u of updates) {
    const sys = result.find(
      (s) => s.name.toLowerCase() === u.name.toLowerCase(),
    );
    if (sys) {
      if (u.addPrinciples?.length)
        sys.principles = [...(sys.principles ?? []), ...u.addPrinciples];
      if (u.addConstraints?.length)
        sys.constraints = [...(sys.constraints ?? []), ...u.addConstraints];
      if (u.addInteractions?.length)
        sys.interactions = [...(sys.interactions ?? []), ...u.addInteractions];
    }
  }
  return result;
}
  ...
  switch (action.type) {
    ...
    case "APPLY_ROUND": {
      ...
      return {
        ...
        systems: applySysUpdates(
          state.systems,
          action.newSystems,
          action.systemUpdates,
        ),
    ...
    case "SET_PREFETCHED":
      return {
        ...state,
        prefetched: { ...state.prefetched, [action.phase]: action.question },
      };
    case "RESTORE":
      return {
        ...state,
...
function ChoiceCard({
  ...
  return (
    ...
    >
      <div
        className={`mt-0.5 w-3.5 h-3.5 rounded shrink-0 border transition-colors flex items-center justify-center ${
          selected
            ? "border-white/40 bg-white/15"
            : "border-white/15 bg-white/3"
        }`}
      >
        {selected && <IconCheck size={10} className="text-white/80" />}
      </div>
      <div className="min-w-0">
        <p
          className={`text-sm font-medium ${selected ? "text-white/90" : "text-white/70"}`}
        >
          {label}
        </p>
...
function DecisionSidebar({
  ...
  return (
    <div
      className="w-[240px] shrink-0 border-r border-white/6 flex flex-col"
      style={{ maxHeight: "calc(100vh - 60px)" }}
    >
      <div className="px-3 py-3 border-b border-white/6">
        <p className="text-[10px] uppercase tracking-[0.15em] text-white/20 font-mono">
          History
        </p>
      </div>
      <div
      ...
      >
        {decisions.length === 0 ? (
          <p className="text-[10px] text-white/15 italic mt-2">
          ...
          <div className="flex flex-col gap-1.5">
            {decisions.map((d, i) => (
              <div
                key={i}
                className="rounded-lg px-2.5 py-2 border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-colors"
              >
                <p className="text-[9px] text-white/20 font-mono mb-0.5">
                  Round {i + 1}
                </p>
...
function DiscoverInner({
  ...
  const inquiryIdRef = useRef<string>(
    initialInquiryId ?? `inquiry-${Date.now()}`,
  );
  ...
  const handleSubmitAnswer = useCallback(async () => {
    ...
    const decision: PremiseDecision = {
    ...
    try {
      ...
      const newSystems = applySysUpdates(
        state.systems,
        result.newSystems,
        result.systemUpdates,
      );
      ...
      dispatch({
        ...
        rules: result.newRules,
        newSystems: result.newSystems,
  ...
  useEffect(() => {
    ...
    if (
      state.decisions.length === prevDecisionCount.current &&
      state.phase === "seed"
    )
      return;
  ...
  const handleCreate = useCallback(() => {
    const { premise, characters, locations, threads, rules, worldSystems } =
      buildPremiseText(
        state.entities,
        state.rules,
        state.worldSummary,
    ...
    storeDispatch({
      ...
      prefillData: {
        ...
        characters: characters.map((c) => ({
          name: c.name,
          role: c.role as "anchor" | "recurring" | "transient",
      ...
      <div className="relative min-h-[calc(100vh-60px)] flex flex-col items-center justify-center px-4">
        ...
        <div className="relative z-10 max-w-lg w-full">
          ...
          <p className="text-[13px] text-white/35 mb-8 leading-relaxed">
            Answer questions to shape what your story is about. Each choice
            refines characters, locations, threads, and rules until the concept
            is ready to write.
          </p>
          ...
          <div className="prompt-glow relative rounded-xl border border-white/8 focus-within:border-white/15 transition-colors duration-200">
            <textarea
              value={state.seed}
              onChange={(e) =>
                dispatch({ type: "SET_SEED", seed: e.target.value })
              }
              rows={3}
    ...
    <>
      <div className="relative flex min-h-[calc(100vh-60px)]">
        ...
        <div
          ...
          style={{ maxHeight: "calc(100vh - 60px)", scrollbarWidth: "none" }}
        >
          <div className="flex-1 flex flex-col justify-center max-w-2xl mx-auto w-full px-8 py-8">
            ...
            <div className="flex items-center justify-center gap-1 mb-8">
              {[
                ...
                },
                {
                  key: "cast" as const,
                  label: "Cast",
                  count: chars.length + locs.length,
                },
                {
                ...
                return (
                  <div key={key} className="flex items-center">
                    ...
                    >
                      ...
                      {count > 0 && (
                        <span
                          className={`text-[8px] font-mono ${hasContent ? "text-emerald-400/25" : "text-white/20"}`}
                        >
                          {count}
                        </span>
                      )}
            ...
            {state.currentQuestion && (
              <div ref={questionRef}>
                <div className="mb-6">
                  ...
                  <h2 className="text-xl font-semibold text-white/90 leading-snug">
            ...
            {state.error && (
              <div className="bg-fate/10 border border-fate/30 rounded-lg px-3 py-2 mt-4">
                <p className="text-xs text-fate/80">{state.error}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div
          className="w-[380px] shrink-0 border-l border-white/6 flex flex-col"
          style={{ maxHeight: "calc(100vh - 60px)", scrollbarWidth: "none" }}
        >
          {/* Title + entity counts */}
          <div className="px-4 py-3 border-b border-white/6">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <span className="text-[10px] text-white/30">
                {state.systems.length} System
                {state.systems.length !== 1 ? "s" : ""}
              </span>
              <span className="text-[10px] text-white/30">
          ...
          >
            {state.phase === "systems" && (
              <div className="px-4 py-3">
                <p className="text-[10px] uppercase tracking-[0.15em] text-white/20 font-mono mb-2">
                  World Systems
                </p>
                {state.systems.length === 0 ? (
                  <p className="text-[11px] text-white/20 italic">
                    No systems yet — answer questions to define how your world
                    works.
                  </p>
                ) : (
                  <div className="flex flex-col gap-3">
                    {state.systems.map((sys, i) => (
                      <div
                        key={i}
                        className="border border-white/6 rounded-lg px-3 py-2.5"
                      >
                        ...
                        {sys.principles?.length > 0 && (
                          <div className="mt-1.5">
                            {sys.principles.map((p, j) => (
                              <p
                                key={j}
                                className="text-[10px] text-white/40 leading-snug pl-2"
                              >
                                • {p}
                              </p>
                            ))}
                        ...
                        {sys.constraints?.length > 0 && (
                          <div className="mt-1">
                            {sys.constraints.map((c, j) => (
                              <p
                                key={j}
                                className="text-[10px] text-amber-400/40 leading-snug pl-2"
                              >
                                ⚠ {c}
                              </p>
                            ))}
                        ...
                        {sys.interactions?.length > 0 && (
                          <div className="mt-1">
                            {sys.interactions.map((ix, j) => (
                              <p
                                key={j}
                                className="text-[10px] text-cyan-400/30 leading-snug pl-2"
                              >
                                ↔ {ix}
                              </p>
                            ))}
            ...
            {state.phase === "rules" && (
              <div className="px-4 py-3">
                ...
                {state.rules.length === 0 ? (
                  <p className="text-[11px] text-white/20 italic">
                    No rules yet — answer questions to define the commandments
                    of your world.
                  </p>
                ) : (
                  <div className="flex flex-col gap-1.5">
                    {state.rules.map((rule, i) => (
                      <div
                        key={i}
                        className="border border-white/6 rounded-lg px-3 py-2"
                      >
                        <p className="text-[11px] text-white/50 leading-snug">
                          <span className="text-white/20 mr-1.5 font-mono text-[10px]">
                            {i + 1}.
                          </span>
                          {rule}
                        </p>
            ...
            {state.phase === "cast" && (
              <div>
                <div className="h-[280px]">
                  <WorldGraph
                    entities={state.entities.filter((e) => e.type !== "thread")}
                    edges={state.edges}
                  />
                </div>
                    ...
                    {chars.map((c, i) => (
                      <div key={i} className="flex items-start gap-2 mb-1.5">
                        <div
                          ...
                          style={{ backgroundColor: TYPE_COLORS.character }}
                        />
                        <div>
                  ...
                  <div className="border-t border-white/6 px-4 py-3">
                    {threads.map((t, i) => (
                      <div key={i} className="flex items-start gap-2 mb-2">
                        <div
                          className="w-2 h-2 rounded-full mt-1 shrink-0"
                          style={{ backgroundColor: TYPE_COLORS.thread }}
                        />
                        <div>
                          <span className="text-[11px] text-white/60 font-medium">
                            {t.name}
