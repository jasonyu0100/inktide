...
<instructions>
  ...
  <rules>
    ...
    <rule name="dialogue-agency" critical="true" hint="Characters with agency SPEAK. The dominant failure mode is treating multi-character interactions as mute encounters resolved through gesture, narrated intent, or interior thought.">
      <directive>When a scene has 2+ participants engaging in ANY substantive exchange — negotiation, confrontation, interview, instruction, alliance, persuasion, accusation, reconciliation, request, refusal — at least one beat's \`what\` field must scaffold the actual conversation: WHO says WHAT to WHOM, with the substance of the exchange, NOT just the outcome.</directive>
      <bad-pattern reason="characters narrated AT, not talking">"Fang Yuan explained the energy signatures to Bai Ning Bing." / "Pan Chong's pleasantries dissolved under Fang Yuan's questions." / "Fang Yuan offered a tactical alliance; Bai Ning Bing nodded sharply." — these treat speech as result-state, not as the actual event. Rewrite so the beat scaffolds quoted lines the prose writer can render verbatim.</bad-pattern>
      <good-pattern>"Fang Yuan walks Bai Ning Bing through the Heaven's Will blind-spot mechanic; she pushes back on the pricing, he concedes one point and holds the rest." / "Pan Chong's first denial collapses when Fang Yuan names Lin Xue's future arrival; the broken man bargains for terms; Fang Yuan grants none." — these tell the prose writer there ARE lines, what they're about, who pushes whom, where the tension lands.</good-pattern>
      <slot-override>If the sampler's slot for an interaction-heavy moment is \`thought\` / \`action\` / \`narration\` and the content is genuinely a multi-character exchange, you may add an extra dialogue beat beyond the sampled slots (server-side extension handles the slot allocation). Better one extra dialogue beat than a scene where allies negotiate in silence.</slot-override>
      <even-non-dialogue-beats>Non-dialogue beats in interaction scenes can and should embed snippets of dialogue when characters are present together. The mechanism names the DOMINANT register, not the exclusive one — an action beat where two characters are working together can land a one-line aside; a thought beat in a conversation can carry an overheard quote. The prose writer reads this scaffolding and decides per beat; the plan should leave room.</even-non-dialogue-beats>
    </rule>
  </rules>
</instructions>`;
}
