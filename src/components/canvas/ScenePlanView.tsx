...
import type { NarrativeState, Scene, BeatPlan, Beat, BeatFn, BeatMechanism } from '@/types/narrative';
...
const MECH_ICONS: Record<string, string> = {
  dialogue: '\u{1F4AC}', thought: '\u{1F4AD}', action: '\u26A1', environment: '\u{1F30D}',
  narration: '\u{1F4D6}', memory: '\u{1F519}', document: '\u{1F4C4}', comic: '\u{1F604}',
};
...
export function ScenePlanView({
  ...
  const [planCache, setPlanCache] = useState<{ plan: BeatPlan | null; status: 'idle' | 'loading' | 'ready' | 'error'; error?: string }>(() =>
    scene.plan ? { plan: scene.plan, status: 'ready' } : { plan: null, status: 'idle' }
  );
  const [reasoning, setReasoning] = useState('');
  const [meta, setMeta] = useState<{ targetBeats: number; estWords: number } | null>(null);

  // Sync when scene changes
  useEffect(() => {
    setPlanCache(scene.plan ? { plan: scene.plan, status: 'ready' } : { plan: null, status: 'idle' });
    setReasoning('');
    setMeta(null);
  }, [scene.id]); // eslint-disable-line react-hooks/exhaustive-deps

  const generatePlan = useCallback(async (guidance?: string) => {
    setPlanCache({ plan: null, status: 'loading' });
    setReasoning('');
    setMeta(null);
    try {
      const plan = await generateScenePlan(narrative, scene, resolvedKeys,
      ...
      setReasoning('');
      setMeta(null);
      dispatch({ type: 'UPDATE_SCENE', sceneId: scene.id, updates: { plan } });
    } catch (err) {
  ...
  const rewritePlan = useCallback(async (guidance: string) => {
    const currentPlan = planCache.plan ?? scene.plan;
    if (!currentPlan) return;
    setPlanCache({ plan: null, status: 'loading' });
    setReasoning('');
    try {
      const plan = await rewriteScenePlan(narrative, scene, resolvedKeys, currentPlan, guidance);
      setPlanCache({ plan, status: 'ready' });
      dispatch({ type: 'UPDATE_SCENE', sceneId: scene.id, updates: { plan } });
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
  ...
  const activePlan: BeatPlan | null = planCache.plan && Array.isArray(planCache.plan.beats) ? planCache.plan : null;

  const updateBeat = useCallback((beatIdx: number, updates: Partial<Beat>) => {
  ...
  const deleteBeat = useCallback((beatIdx: number) => {
    if (!activePlan) return;
  ...
  const insertBeat = useCallback((afterIdx: number) => {
    if (!activePlan) return;
  ...
  const updateAnchor = useCallback((anchorIdx: number, value: string) => {
    if (!activePlan) return;
  ...
  const deleteAnchor = useCallback((anchorIdx: number) => {
    if (!activePlan) return;
      ...
      <div className="max-w-2xl mx-auto px-8 pt-6 pb-32">
        ...
        {isLoading && (
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[10px] text-text-dim">Generating plan...</span>
              {meta && (
        ...
        {activePlan && !isLoading && !hasError && (
          <div className="space-y-3">
                  ...
                  <div key={i} className="group relative flex gap-3 py-2.5 px-1 hover:bg-white/2 rounded-lg transition-colors">
                    ...
                    <div className="flex-1 min-w-0 -mt-0.5">
                      <div className="flex items-center gap-2 mb-1">
                        <select value={beat.fn} onChange={(e) => updateBeat(i, { fn: e.target.value as BeatFn })}
                          ...
                          {BEAT_FN_LIST.map((fn) => (<option key={fn} value={fn} className="bg-bg-panel text-text-primary">{fn}</option>))}
                        </select>
                        <select value={beat.mechanism} onChange={(e) => updateBeat(i, { mechanism: e.target.value as BeatMechanism })}
                          className="text-[9px] text-text-dim/50 bg-transparent border-none outline-none cursor-pointer appearance-none pr-3">
                          {BEAT_MECHANISM_LIST.map((m) => (<option key={m} value={m} className="bg-bg-panel text-text-primary">{(MECH_ICONS[m] ?? '\u2022') + ' ' + m}</option>))}
                        </select>
                        <div className="ml-auto flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => insertBeat(i)} className="w-5 h-5 flex items-center justify-center rounded hover:bg-emerald-500/15 text-text-dim/40 hover:text-emerald-400 transition-all" title="Insert beat after">
                            <IconPlus size={10} />
                          </button>
                          <button onClick={() => deleteBeat(i)} className="w-5 h-5 flex items-center justify-center rounded hover:bg-red-500/15 text-text-dim/40 hover:text-red-400 transition-all" title="Delete beat">
                            <IconClose size={10} />
                          </button>
                      ...
                      <p contentEditable suppressContentEditableWarning
                        ...
                        onBlur={(e) => { const text = e.currentTarget.textContent ?? ''; if (text !== beat.what) updateBeat(i, { what: text }); }}>
                        {beat.what}
                      </p>
                      <p contentEditable suppressContentEditableWarning
                        className="text-[10px] text-text-dim/60 mt-0.5 italic outline-none focus:bg-white/3 rounded px-1 -mx-1"
                        onBlur={(e) => { const text = e.currentTarget.textContent ?? ''; if (text !== beat.anchor) updateBeat(i, { anchor: text }); }}>
                        {beat.anchor}
                      </p>
                    </div>
                  </div>
            ...
            <button onClick={() => insertBeat(activePlan.beats.length - 1)}
            ...
            <div className="mt-2 pt-3 border-t border-white/5">
              ...
              <div className="space-y-2">
                {activePlan.anchors.map((a, i) => (
                  <div key={i} className="group/anchor pl-3 border-l-2 border-amber-400/30 flex items-start gap-1">
                    <p contentEditable suppressContentEditableWarning
                      ...
                      onBlur={(e) => { const text = e.currentTarget.textContent ?? ''; if (text !== a) updateAnchor(i, text); }}>
                      {a}
                    </p>
                    <button onClick={() => deleteAnchor(i)}
                ...
                <button onClick={addAnchor} className="text-[9px] text-amber-400/30 hover:text-amber-400/60 transition-colors">
                  + Add anchor line
                </button>
              </div>
            </div>
